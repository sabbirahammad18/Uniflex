...
const Profile = () => {
    ...
    const handlePlotSearch = async () => {
        const {sectorNo, block, roadNo, plotNo} = plotForm;

        if (!sectorNo.trim() || !block || !roadNo.trim() || !plotNo.trim()) {
            setPlotSearchError("Please fill in all fields before searching.");
        ...
        setPlotSearchError("");
        setPlotSearchResult(null);
        setPlotBreakdown(null);

        try {
            const response = await triggerPlotSearch({
                sector_no: sectorNo.trim(),
                block,
                road_no: roadNo.trim(),
                plot: plotNo.trim()
            }).unwrap();
            setPlotBreakdown({sector: sectorNo.trim(), block, road: roadNo.trim(), plotShare: plotNo.trim()});
    ...
    return (
        <div className="bg-white pb-16 font-sans text-slate-950">
            <main className="mx-auto w-full max-w-107.5 px-4 py-8 space-y-6">
                ...
                <span
                    className="rounded-full bg-blue-50 border border-blue-100 px-3 py-1 text-xs font-medium text-blue-700 capitalize">
                  {designation}
                </span>
                                <span className="text-sm text-slate-400">ID: {displayUid}</span>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                {!superAdminUser ? (
                    <section className="rounded-xl bg-[#07277F] p-6 text-white shadow-lg">
                ...
                <section className="grid grid-cols-2 gap-4">
                  ...
                  History
                            ...
                            >
                                <div
                                    className="w-10 h-10 rounded-lg bg-indigo-100 grid place-items-center text-indigo-600">
                                    <span className="material-symbols-outlined text-[23px]">account_tree</span>
                                </div>
                                <span className="text-sm leading-4 font-medium text-[#00176b]">
                  Employee
                  <br/>
                  Tree
                            ...
                            >
                                <div className="w-10 h-10 rounded-lg bg-blue-100 grid place-items-center text-blue-600">
                                    <span className="material-symbols-outlined text-[23px]">payments</span>
                                </div>
                                <span className="text-sm leading-4 font-medium text-[#00176b]">Commission</span>
                            </Link>
                            ...
                            >
                                ...
                                <span className="text-sm leading-4 font-medium text-[#00176b]">Achievement</span>
                            </Link>
                        </>
                    ) : null}
                </section>
                ...
                travel_explore
                        ...
                        <div className="mt-5 grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <input
                                    value={plotForm.sectorNo}
                                    onChange={(e) => {
                                        dispatchPlotForm({sectorNo: e.target.value});
                                        setPlotSearchError("");
                                    }}
                                    placeholder="e.g. 01"
                                    className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm outline-none transition focus:border-[#07277F] focus:bg-white focus:ring-4 focus:ring-blue-100"
                                    type="text"
                                    autoComplete="off"
                                />
                            </div>
                            ...
                            <div className="relative">
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        dispatchPlotForm({block: e.target.value});
                                        setPlotSearchError("");
                                    }}
                                    className="h-12 w-full appearance-none rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm outline-none transition focus:border-[#07277F] focus:bg-white focus:ring-4 focus:ring-blue-100"
                                >
                  ...
                  expand_more
                            ...
                            <div>
                                ...
                                <input
                                    value={plotForm.roadNo}
                                    onChange={(e) => {
                                        dispatchPlotForm({roadNo: e.target.value});
                                        setPlotSearchError("");
                                    }}
                                    placeholder="e.g. 07"
                                    className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm outline-none transition focus:border-[#07277F] focus:bg-white focus:ring-4 focus:ring-blue-100"
                            ...
                            <div>
                                ...
                                <input
                                    value={plotForm.plotNo}
                                    onChange={(e) => {
                                        dispatchPlotForm({plotNo: e.target.value});
                                        setPlotSearchError("");
                                    }}
                        ...
                        <div className="mt-3">
                            <button
                                ...
                                className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#07277F] px-6 text-sm font-bold text-white shadow-lg transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60"
                                type="button"
                            >
                ...
                <div className="grid grid-cols-1 gap-5">
                    ...
                    {formatCurrency(paymentSummary?.installment_amount)}
                                ...
                                <div className="grid grid-cols-[1fr_auto] items-center border-t border-slate-100 pt-5">
                                    <span className="text-base font-extrabold text-[#00176b]">Remaining</span>
                                    <span className="text-base font-extrabold text-red-600">
                    {formatCurrency(paymentSummary?.remaining_amount)}
                  </span>
                                </div>
                                <p className="text-xs font-semibold text-slate-500">
                                    Plot size: {formatPlainNumber(paymentSummary?.plot_size_khata)} Khata
                                </p>
                            </div>
                        </section>
                    ) : (
                        ...
                        <span className="text-sm font-extrabold text-[#00176b]">
                          ...
                          {openType === item.category_name ? "expand_less" : "expand_more"}
                        </span>
                                            </div>
                                        </button>
                                                    ...
                                                    >
                                                        <div className="flex items-start justify-between gap-3">
                                                            <div className="flex flex-col mt-2">
                                                                ...
                                                                <div
                                                                    className="mt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500">
                                                                    <span>ID: {customer.customer_uid || "N/A"}</span>
                                                                </div>
                                                            </div>
                                                            <div className="flex flex-col items-end">
                                <span
                                    className="rounded-xl bg-blue-100 px-3 py-1 text-sm font-extrabold text-[#00176b]">
                                  {formatCurrency(customer.amount)}
                                </span>
                                                                <span
                                                                    className="text-label-md font-semibold mt-2 opacity-60">
                                  {customer.date || "N/A"}
                                </span>
                                                            </div>
                                                        </div>
                                ...
                                {
                                    marketingUser ? (<div
                                            className="grid grid-cols-[1fr_auto] items-center border-t border-slate-100 pt-5">
                                            ...
                                            <div className="bg-white rounded-2xl border border-green-100 p-5 shadow-sm hover:shadow-md transition-all">
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <h3
                                                            className=" font-bold text-green-600 mt-2 whitespace-nowrap"
                                                            title={`৳ ${Number(earnings?.total_income || 0).toLocaleString()}`}
                                                        >
                                                            {formatCurrency(earnings?.total_income)}
                                                        </h3>
                                                    </div>
                                            ...
                                            <div className="bg-white rounded-2xl border border-red-100 p-5 shadow-sm hover:shadow-md transition-all">
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <h3
                                                            className=" font-bold text-red-600 mt-2 whitespace-nowrap"
                                                            title={`৳ ${Number(earnings?.total_expense || 0).toLocaleString()}`}
                                                        >
                                                            {formatCurrency(earnings?.total_expense)}
                                                        </h3>
                                                    </div>
