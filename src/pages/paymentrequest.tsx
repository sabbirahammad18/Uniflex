...
} from "@/queries/payoutQuery";
import { formatCurrency, getApiErrorMessage } from "@/utils/format";
import type { PayoutRequestPayload } from "@/queries/types";

const BalanceRequestCard = () => {
  const { data: balance } = useGetPayoutBalanceQuery();
  const [createRequest, { isLoading }] = useCreatePayoutRequestMutation();
  const [requestBalance, setRequestBalance] = useState("");
  const [method, setMethod] =
    useState<PayoutRequestPayload["payment_method"]>("Cash");
  const [message, setMessage] = useState("");
  const [errorMessage, setErrorMessage] = useState("");

  const currentBalance = balance?.data.remaining_balance || 0;
  const requestAmount = Number(requestBalance) || 0;
  const remainingBalance = Math.max(currentBalance - requestAmount, 0);
  const canSubmit = requestAmount > 0 && requestAmount <= currentBalance;
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setErrorMessage(getApiErrorMessage(error, "Request submit failed"));
    }
  };
    ...
    <div className="w-full max-w-107.5 p-3">
      <div className="relative overflow-hidden">
        ...
        <form
          ...
          className="relative z-10 mt-6 space-y-4 rounded-t-[34px] bg-white p-5"
        >
          {message && (
          ...
          <div>
            ...
            <div className="flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-1 focus-within:border-[#1239d8] focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-100">
              ...
              <input
                ...
                onChange={(event) => setRequestBalance(event.target.value)}
                placeholder="0.00"
                min={1}
          ...
          <div>
            ...
            <div className="relative rounded border border-slate-200 bg-slate-50 px-4 shadow-inner focus-within:border-[#1239d8] focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-100">
              <select
                value={method}
                onChange={(event) =>
                  setMethod(event.target.value as PayoutRequestPayload["payment_method"])
                }
                className="w-full appearance-none bg-transparent py-2 px-4 text-sm font-bold text-black outline-none"
              >
          ...
          <button
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-[#07277F] px-5 py-4 text-sm font-black text-white hover:bg-[#00228f] disabled:cursor-not-allowed disabled:opacity-60"
